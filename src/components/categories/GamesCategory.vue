...
// Tracking added game Ids to disable add button
const addedGameIds = ref(new Set());
...
async function getUserGameIds() {
  ...
  snapshot.forEach(doc => {
    const data = doc.data();
    if (data?.media_type === 'movie' && data?.rawg_id) {
      ids.add(data.rawg_id);
    }
  });

  addedGameIds.value = ids;
}

// Add movie to backlog
const addtoBacklog = async (gameData) => {
  ...
  try {
    ...
    const q = fsQuery(
      ...
      where('rawg_id', '==', gameData.id));

    const checkExisting = await getDocs(q);

    if (!checkExisting.empty) {
      addedGameIds.value.add(gameData.id); // In case user added after load
    ...
    const state = ensureState(gameData.id);
    ...
    // To hide right away
    addedGameIds.value.add(gameData.id);
